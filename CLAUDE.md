...
| Layer | Tech |
|-------|------|
| App | Expo SDK 54, React Native 0.81, Expo Router 6, TypeScript 5 (strict) |
...
- カラー: navy (#0f172a) + coral (#f43f5e) — `constants/colors.ts`
- 認証トークンは `expo-secure-store` に保存
- タブレット対応: `useWindowDimensions` で `width >= 768` 判定
- DB スキーマ変更は `supabase/migrations/` に SQL ファイル追加

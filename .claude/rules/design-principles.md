...
> 対象: que-app（Expo / React Native）。que-site（marketing）は別ルール。
...
Que の日本語コピーは **アスピレーショナル形（願望・呼びかけ・提案）** で統一。断言形・押し付け形は禁止。
...
- ❌ shadcn/ui / Tailwind / Geist / Lucide を que-app に持ち込む（Apple純正から外れる）
- ❌ 装飾的な影・グラデーション・枠線
...
- ❌ 不正解時の shake / 揺れアニメ（Stripe 流の遊び心は OK だが、Que では揺らさない）
